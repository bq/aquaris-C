...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if ((err == tfa_error_ok) && (tfa98xx->set_mtp_cal)) {
		...
		if (err_cal != tfa_error_ok) {
			printk("Error, setting calibration value in mtp, err=%d\n", err_cal);
		} else {
...
static int tfa98xx_dbgfs_otc_get(void *data, u64 *val)
{
	...
	if (value < 0) {
		printk("[0x%x] Unable to check DSP access: %d\n", tfa98xx->i2c->addr, value);
		return -EIO;
	}
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (val != 0 && val != 1) {
		printk("[0x%x] Unexpected value %llu\n", tfa98xx->i2c->addr, val);
		return -EINVAL;
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (val != 0) {
		printk("[0x%x] Can only clear MTPEX (0 value expected)\n", tfa98xx->i2c->addr);
		return -EINVAL;
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (ret) {
		printk("[0x%x] Calibration start failed (%d)\n", tfa98xx->i2c->addr, ret);
		return -EIO;
	} else {
		printk("[0x%x] Calibration started\n", tfa98xx->i2c->addr);
	}
				     ...
				     loff_t *ppos)
{
	...
	if (ret) {
		ret = -EIO;
		printk("[0x%x] calibration failed\n", tfa98xx->i2c->addr);
		goto r_c_err;
	}
	...
	if (!str) {
		ret = -ENOMEM;
		printk("[0x%x] memory allocation failed\n", tfa98xx->i2c->addr);
		goto r_c_err;
	}
	...
	printk("[0x%x] calib_done: %s", tfa98xx->i2c->addr, str);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		printk("[0x%x] Manual triggering of dsp start...\n", tfa98xx->i2c->addr);
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
		mutex_unlock(&tfa98xx->dsp_lock);
		printk("[0x%x] tfa_dev_start complete: %d\n",  tfa98xx->i2c->addr, ret);
	} else if (!strncmp(buf, stop_cmd, sizeof(stop_cmd) - 1)) {
		printk("[0x%x] Manual triggering of dsp stop...\n",  tfa98xx->i2c->addr);
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa_dev_stop(tfa98xx->tfa);
		mutex_unlock(&tfa98xx->dsp_lock);
		printk("[0x%x] tfa_dev_stop complete: %d\n",  tfa98xx->i2c->addr, ret);
	} else if (!strncmp(buf, mon_start_cmd, sizeof(mon_start_cmd) - 1)) {
		printk("[0x%x] Manual start of monitor thread...\n",  tfa98xx->i2c->addr);
		queue_delayed_work(tfa98xx->tfa98xx_wq,
					&tfa98xx->monitor_work, HZ);
	} else if (!strncmp(buf, mon_stop_cmd, sizeof(mon_stop_cmd) - 1)) {
		printk("[0x%x] Manual stop of monitor thread...\n",  tfa98xx->i2c->addr);
				     ...
				     loff_t *ppos)
{
	...
	printk("[0x%x] fw_state : %s", tfa98xx->i2c->addr, str);
				     ...
				     loff_t *ppos)
{
	...
	if (tfa98xx->tfa == NULL) {
		printk("[0x%x] dsp is not available\n", tfa98xx->i2c->addr);
		return -ENODEV;
	}
	...
	if (buffer == NULL) {
		printk("[0x%x] can not allocate memory\n", tfa98xx->i2c->addr);
		return -ENOMEM;
	}
	...
	if (error != Tfa98xx_Error_Ok) {
		printk("[0x%x] dsp_msg_read error: %d\n", tfa98xx->i2c->addr, error);
		kfree(buffer);
		return -EFAULT;
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		...
		if (error != Tfa98xx_Error_Ok) {
			printk("[0x%x] tfaContWriteFile error: %d\n", tfa98xx->i2c->addr, error);
			err = -EIO;
		}
	} else {
		error = dsp_msg(tfa98xx->tfa, msg_file->size, msg_file->data);
		if (error != Tfa98xx_Error_Ok) {
			printk("[0x%x] dsp_msg error: %d\n", tfa98xx->i2c->addr, error);
			err = -EIO;
		}
	}
	mutex_unlock(&tfa98xx->dsp_lock);
...
static int add_sr_to_profile(struct tfa98xx *tfa98xx, char *basename, int len, int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if ((len == bprof->len) && (0 == strncmp(bprof->basename, basename, len))) {
			...
			if (!sr) {
				printk("unable to identify supported sample rate for %s\n", bprof->basename);
				return -1;
			}
			...
			if (idx < 0 || idx >= TFA98XX_NUM_RATES) {
				printk("invalid index for samplerate %d\n", idx);
				return -1;
			}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile < 0) {
		printk("tfa98xx: tfa98xx_get_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n", profile, mixer_profile, tfa98xx->rate);
		return -EINVAL;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	profile = get_profile_id_for_sr(mixer_profile, tfa98xx->rate);
	if (profile < 0) {
		printk("tfa98xx: tfa98xx_set_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n", profile, mixer_profile, tfa98xx->rate);
		return -EINVAL;
	}
...
#endif
		printk("%d: vstep:%d, (control value: %d) - profile %d\n",
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	if (profile < 0) {
		printk("tfa98xx: tfa98xx_info_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n", profile, mixer_profile, tfa98xx->rate);
		return -EINVAL;
	}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (prof_idx < 0) {
		printk("tfa98xx: sample rate [%d] not supported for this mixer profile [%d].\n", tfa98xx->rate, new_profile);
		return 0;
	}
					...
					unsigned short value)
{
	...
	if (!tfa98xx || !tfa98xx->regmap) {
		printk("No tfa98xx regmap available\n");
...
retry:
	...
	printk("  WR-RAW (len=%d) Error I2C send size mismatch %d\n", len, ret);
	error = Tfa98xx_Error_Fail;
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (prof_idx < 0) {
		printk("tfa98xx: invalid sample rate %d.\n", rate);
		return -EINVAL;
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		case 0x88: /* tfa9888 */
			...
			break;
		case 0x13: /* tfa9912 */
			printk("TFA9912 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			/* tfa98xx->flags |= TFA98XX_FLAG_TAPDET_AVAILABLE; */
			break;
		case 0x94: /* tfa9894 */
			printk("TFA9894 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
...
static int __init tfa98xx_i2c_init(void)
{
	...
	if (!tfa98xx_cache) {
		printk("tfa98xx can't create memory pool\n");
		ret = -ENOMEM;
	}
