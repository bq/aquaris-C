...
static ssize_t read_proc_info(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	int cnt=0;
	char *buf = NULL;
	buf = kzalloc(128,GFP_KERNEL);
	cnt = sprintf(buf,"[Vendor]%s,%s\n","sunwear","9085G");
	cnt = simple_read_from_buffer(page,size,ppos,buf,cnt);
	kfree(buf);
	printk("finger--read\n");		
	return cnt;
...
static int finger_create_proc_entry(void)
{
   struct proc_dir_entry * proc_entry_finger;
   proc_entry_finger = proc_create_data("finger_info", 0444, NULL, &sf_fops, NULL);
    if (IS_ERR_OR_NULL(proc_entry_finger))
    {
...
static int sf_probe(sf_device_t *dev)
{
	...
	sf_ctl_dev.wait_finger_down=false;
	sf_ctl_dev.fb_black=false;
	INIT_WORK(&sf_ctl_dev.work,notification_work);
	finger_create_proc_entry();
#endif
