...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    /* report event key */
    if (gesture != -1) {
        FTS_DEBUG("swb.Gesture Code=%d", gesture);
...
void fts_gesture_recovery(struct i2c_client *client)
{
    if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        ...
        fts_i2c_write_reg(client, 0xD1, 0x10);
        fts_i2c_write_reg(client, 0xD2, 0x00);
        fts_i2c_write_reg(client, 0xD5, 0x00);
        fts_i2c_write_reg(client, 0xD6, 0x00);
        fts_i2c_write_reg(client, 0xD7, 0x00);
